...
export const GamificationHeader = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden rounded-[24px] bg-slate-900/90 dark:bg-slate-950/70 border border-slate-800 dark:border-white/10 p-6 text-white shadow-xl backdrop-blur-xl flex items-center gap-5 group transition-all duration-300 hover:shadow-2xl hover:border-indigo-500/30">
        {/* Glow Node */}
        <div className="absolute -top-12 -left-12 h-32 w-32 rounded-full bg-indigo-500/10 blur-[40px] group-hover:bg-indigo-500/20 transition-all duration-500" />
        ...
        <div className="relative flex-shrink-0 w-20 h-20 flex items-center justify-center">
          ...
          <div className="w-14 h-14 rounded-full bg-slate-950 border border-slate-800 flex flex-col items-center justify-center shadow-inner relative z-10">
            <span className="text-[8px] font-bold uppercase tracking-[0.1em] text-slate-400">LVL</span>
            <span className="text-xl font-black bg-gradient-to-b from-white to-slate-300 bg-clip-text text-transparent leading-none">
              {level}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-1 z-10">
          <div className="flex items-center gap-2">
            <h4 className="text-lg font-bold font-display tracking-tight bg-gradient-to-r from-white via-indigo-100 to-indigo-200 bg-clip-text text-transparent">
              {rankName}
            </h4>
          </div>
        ...
        <div className="absolute top-4 right-4 text-indigo-500/30 group-hover:text-indigo-400/80 transition-colors duration-300">
          <Sparkles className="h-4 w-4 animate-pulse" />
        </div>
      </div>

      {/* CARD 2: XP Tracking & Streaks */}
      <div className="relative overflow-hidden rounded-[24px] bg-slate-900/90 dark:bg-slate-950/70 border border-slate-800 dark:border-white/10 p-6 text-white shadow-xl backdrop-blur-xl flex flex-col justify-between gap-4 group transition-all duration-300 hover:shadow-2xl hover:border-cyan-500/30">
        {/* Glow Node */}
        <div className="absolute -bottom-12 -right-12 h-32 w-32 rounded-full bg-cyan-500/10 blur-[40px] group-hover:bg-cyan-500/20 transition-all duration-500" />
        
        <div className="flex items-center justify-between gap-4 z-10">
          <div className="space-y-0.5">
            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400">Experience Tracker</span>
            <div className="flex items-baseline gap-1">
              <span className="text-lg font-bold font-mono text-cyan-300">{xp.toLocaleString()}</span>
              <span className="text-xs text-slate-400">/ {xpForNextLevel.toLocaleString()} XP</span>
            </div>
          </div>

          {/* Streak Badge */}
          {savingsStreak > 0 ? (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-400 shadow-[0_4px_12px_rgba(249,115,22,0.05)]">
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-950 border border-slate-800 text-slate-500">
      ...
      <div className="relative overflow-hidden rounded-[24px] bg-slate-900/90 dark:bg-slate-950/70 border border-slate-800 dark:border-white/10 p-6 text-white shadow-xl backdrop-blur-xl flex flex-col justify-between gap-3 group transition-all duration-300 hover:shadow-2xl hover:border-violet-500/30">
        {/* Glow Node */}
        <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full bg-violet-500/10 blur-[40px] group-hover:bg-violet-500/20 transition-all duration-500" />
        
        <div className="z-10">
          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 block mb-3">Badge Cabinet</span>
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    isEarned
                      ? `bg-gradient-to-br ${b.color} text-white border-white/20 shadow-[0_4px_12px_rgba(0,0,0,0.2)]`
        ...
        <div className="flex justify-between items-center text-[9px] font-bold uppercase tracking-wider text-slate-500 z-10">

...
import { MessageSquarePlus, X, Star, Send, Bug, ChevronRight, CheckCircle2 } from "lucide-react";
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="w-[340px] bg-white/95 backdrop-blur-xl border border-white/30 rounded-3xl shadow-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-primary/15 to-primary/5 px-5 py-4 flex items-center justify-between border-b border-white/20">
              <div>
                <h3 className="font-display font-bold text-gray-800 text-sm">Share your thoughts</h3>
                <p className="text-xs text-muted-foreground mt-0.5">Help us build a better Spend Sense</p>
              </div>
              <button
                onClick={handleClose}
            ...
            <div className="flex gap-1 p-3 bg-muted/30">
              {(["feedback", "bug"] as Tab[]).map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`flex-1 py-2 px-3 rounded-xl text-xs font-semibold transition-all capitalize flex items-center justify-center gap-1.5 ${
                    tab === t
                      ? "bg-white text-primary shadow-sm"
                      : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {t === "feedback" ? <Star className="h-3.5 w-3.5" /> : <Bug className="h-3.5 w-3.5" />}
              ...
              {tab === "feedback" ? (
                <>
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-gray-600">How would you rate your experience?</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-right text-[10px] text-muted-foreground">{feedbackText.length}/500</p>
                  </div>

                  {/* Optional email */}
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-gray-600">Email for follow-up <span className="font-normal text-muted-foreground">(optional)</span></Label>
                    <Input
                      type="email"
                ...
                <>
                      ...
                      {(["Low", "Medium", "High", "Critical"] as BugSeverity[]).map((s) => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all ${
                            severity === s ? severityColors[s] + " shadow-sm ring-2 ring-offset-1 ring-current/20" : "bg-white text-gray-500 border-gray-200"
                          }`}
                        >
                          {s}
                        </button>
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-gray-600">Steps to reproduce</Label>
                    <Textarea
                      placeholder="1. Go to...\n2. Click on...\n3. Notice that..."
                      value={bugSteps}
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-gray-600">What did you expect?</Label>
                    <Textarea
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-gray-600">What actually happened?</Label>
                    <Textarea
                  ...
                  <div className="bg-muted/40 rounded-xl px-3 py-2 text-[10px] text-muted-foreground space-y-0.5">
                    <p>📍 Auto-captured: <span className="font-mono">{window.location.pathname}</span></p>

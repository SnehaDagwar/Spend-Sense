...
export const loadTokens = async () => {
  try {
    _accessToken = await SecureStore.getItemAsync("spend_sense_access_token");
    _refreshToken = await SecureStore.getItemAsync("spend_sense_refresh_token");
  } catch (e) {
...
export const setTokens = async (accessToken: string, refreshToken: string) => {
  ...
  try {
    await SecureStore.setItemAsync("spend_sense_access_token", accessToken);
    await SecureStore.setItemAsync("spend_sense_refresh_token", refreshToken);
  } catch (e) {
...
export const clearTokens = async () => {
  ...
  _refreshToken = null;
  try {
    await SecureStore.deleteItemAsync("spend_sense_access_token");
    await SecureStore.deleteItemAsync("spend_sense_refresh_token");
  } catch (e) {
...
// Global event subscription equivalent for React Native
type AuthListener = () => void;
const sessionExpiredListeners = new Set<AuthListener>();

export const subscribeToSessionExpired = (listener: AuthListener) => {
